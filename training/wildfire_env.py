...
class WildfireEnv(gym.Env):
    def __init__(self, df, config):
        ...
        # -----------------------------
        # Extract config parameters locally 
        # -----------------------------
        self.config = config

        self.battery_energy_dict = config["Initial_Battery_Levels"]
        self.max_missing_fire_min = config["max_missing_fire_min"]
        self.reserved_energy = config["Energy_Constraints"]["reserved_energy"]

        self.E_proc_rl = config["Energy_Constraints"]["E_proc_rl"]
        self.E_temp_humidity_sensor = config["Energy_Constraints"]["E_temp_humidity_sensor"]
        self.E_anemometer_sensor = config["Energy_Constraints"]["E_anemometer_sensor"]
        self.E_proc_ml = config["Energy_Constraints"]["E_proc_ml"]
        self.E_camera_host = config["Energy_Constraints"]["E_camera_host"]
        self.E_comm = config["Energy_Constraints"]["E_comm"]

        self.P_temp_humidity_standby = config["Standby_Power_Components"]["P_temp_humidity_standby"]
        self.P_anemometer_standby = config["Standby_Power_Components"]["P_anemometer_standby"]
        self.P_camera_standby = config["Standby_Power_Components"]["P_camera_standby"]
        self.P_comm_standby = config["Standby_Power_Components"]["P_comm_standby"]

        self.E_battery_leakage_percentage = config["E_battery_leakage_percentage"]
        self.harvested_energy_loss = config["harvested_energy_loss"]

        self.beta = config["Reward_Params"]["beta"]
        self.k1 = config["Reward_Params"]["k1"]
        self.alpha_B = config["Reward_Params"]["alpha_B"]
        self.R_min = config["Reward_Params"]["R_min"]

        self.max_sampling_time = config["TD3_params"]["max_sampling_time"]
        self.min_sampling_time = config["TD3_params"]["min_sampling_time"]
        ...
        battery_choice = np.random.choice(list(self.battery_energy_dict.keys()))
        initial_energy = self.battery_energy_dict[battery_choice]
        ...
        self.energy_budget = initial_energy - self.reserved_energy
        ...
        self.action_space = spaces.Box(low=np.array([self.min_sampling_time]), high=np.array([self.max_sampling_time]), dtype=np.float32)

    def reset(self):
        
        self.current_sensor = np.random.choice(list(self.sensor_selection_count.keys()))  # Pick new sensor
        ...
        # Randomly pick from safe (pre-fire) timestamps
        self.current_step = np.random.choice(allowed_indices)
        
        self.last_image_timestamp = self.sensor_data["Timestamp"].iloc[self.current_step]
        
        self.data_length = len(self.sensor_data) - (self.current_step + 1)     
        self.fire_detection_time = None
        self.battery_depletion_time = None
    ...
    def get_state(self):
        ...
        return np.array([
            ...
            row["HDWI"],
            row["solar_energy"],
            normalize_feature(self.last_sampling_time, 1, self.max_sampling_time),
            normalize_feature(self.battery_energy, 0, self.max_battery_energy),
            # normalize_feature(self.energy_budget, 0, self.max_battery_energy - self.reserved_energy),
    ...
    def step(self, action):
        ...
        if take_picture:
            """ ml_result = np.random.choice([1, 0], p=[TP_rate, 1 - TP_rate]) if row["Label"] == 1 else \
                        np.random.choice([1, 0], p=[FP_rate, 1 - FP_rate]) """
            ml_result = 1
        ...
        if not skipped_data.empty and "Label" in skipped_data.columns:
            self.missed_fire = any((skipped_data["Label"] == 1) &
                            ((row["Timestamp"] - skipped_data["Timestamp"]).dt.total_seconds() / 60 > self.max_missing_fire_min))
        ...
        total_harvested_energy = (fire_rows["solar_energy"] * self.harvested_energy_loss).sum() if not fire_rows.empty else 0
        
        time_skipped_hours = self.last_sampling_time / 60  # Convert to hours
        standby_power_used = (
            self.P_temp_humidity_standby + self.P_anemometer_standby +
            self.P_camera_standby + self.P_comm_standby
        ) * time_skipped_hours  # Multiply by time skipped

        # Energy management with standby power
        energy_used = (
            self.E_proc_rl + self.E_temp_humidity_sensor + self.E_anemometer_sensor + (self.E_proc_ml + self.E_camera_host if take_picture else 0) +
            standby_power_used + (self.E_comm if ml_result and take_picture else 0)
        )
        ...
        if self.battery_depletion_time is None:
            ...
            E_used = (
                self.E_proc_rl + self.E_temp_humidity_sensor + self.E_anemometer_sensor + (self.E_proc_ml + self.E_camera_host if take_picture else 0) +
                (self.E_comm if ml_result and take_picture else 0)
            )
            for i, ts in enumerate(fire_rows["Timestamp"]):
                harvested = fire_rows.iloc[i]["solar_energy"] * self.harvested_energy_loss
                ...
                current_batt *= (1 - self.E_battery_leakage_percentage)
                max_batt_energy *= (1 - self.E_battery_leakage_percentage)
                current_batt = max(0, min(max_batt_energy, current_batt))

                if current_batt - self.reserved_energy <= 0:
                    self.battery_depletion_time = ts
                    break  # First time battery hits 0
                
            if  current_batt - self.reserved_energy > 0 and current_batt - E_used - self.reserved_energy <= 0:
        ...
        self.battery_energy *= (1 - self.E_battery_leakage_percentage)  # Apply battery leakage loss
        self.max_battery_energy *= (1 - self.E_battery_leakage_percentage)  # Apply battery leakage loss
        ...
        self.energy_budget = max(0, self.battery_energy - self.reserved_energy)  
        ...
        self.reward  = self.beta * self.last_sampling_time + (1 - self.beta) * self.reward
        
        step_reward = self.last_sampling_time * (1 - 2 * take_picture) / self.data_length * 100
        ...
        if done:
            ...
            final_reward, reason = self.calculate_final_reward(avg_sampling_time) # -k1 * self.reward, "xx" #self.calculate_final_reward()
    ...
    def calculate_final_reward(self, avg_sampling_time):
        """Calculate the reward at the end of the episode."""
        if self.battery_depletion_time and (not self.fire_start_time or self.battery_depletion_time < self.fire_start_time):
            ...
            reward = -self.alpha_B * (1 / (t_deplete_minus_t_start)) - self.R_min
            reason = "case1"
            print('reward case1 ', reward, "t_deplete_minus_t_start", t_deplete_minus_t_start)
        else:       
            reward = -self.k1 * self.reward # - k2 * self.missed_fire_time
            
            # Case 2/3: Fire detected within allowed time or not detected
            """ if self.fire_detection_time:
                t_detection = (self.fire_detection_time - self.fire_start_time).total_seconds() / 60
                if  t_detection <= max_missing_fire_min:
                    reward = R_max_d * (1 - (t_detection / max_missing_fire_min) ** alpha_d) 
                else:
                    reward = R_max_m * (1 - (t_detection / max_missing_fire_min)) ** alpha_m
            else:
                reward = R_max_m * (1 - (720 / max_missing_fire_min)) ** alpha_m """
    ...
    def plot_episode_metrics(self, reason, final_reward):
        ...
        folder = f"Data/episode_plots_step_reward{self.file_name}"
        ...
        df2.to_csv(f"{folder}/episode_{self.episode_counter}.csv", index=False)  # index=False avoids adding an extra index column
        ...
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(f"{folder}/episode_{self.episode_counter}.png", dpi=300)
        plt.close()
